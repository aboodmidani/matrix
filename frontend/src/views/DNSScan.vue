    ...
    <transition name="fade">
      <div v-if="loading" class="mt-8 bg-black border-2 border-green-500 rounded-lg p-6">
        <div class="flex items-center justify-between mb-4">
          <span class="text-green-400 font-mono text-lg font-bold tracking-wider">
            {{ processingMessage }}
          </span>
        ...
        <div class="border border-green-600 rounded p-4 bg-gray-900">
          <div class="text-green-400 font-mono text-xs mb-3">
            <span class="text-green-300">root@matrix</span>:<span class="text-blue-400">~</span>$ {{ commandDisplay }}
          </div>
          <!-- Terminal-like progress bar -->
          <div class="relative h-8 bg-gray-800 rounded border border-green-700 overflow-hidden">
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="flex space-x-1">
                <template v-for="i in 20" :key="i">
                  <div 
                    class="w-1 h-4 bg-green-500 rounded-sm transition-all duration-300"
                    :class="{ 'h-6 bg-green-400': loading, 'h-2 bg-green-700': !loading }"
          ...
          <div class="text-center text-green-400 font-mono text-xs mt-2">
            {{ executingLabel }}
          </div>
        </div>
      </div>
    ...
    <transition name="slide-up">
      <div v-if="results" class="mt-8 bg-black bg-opacity-90 border border-green-500 rounded-lg p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
          ...
          <button @click="downloadReport" class="px-4 py-2 bg-green-600 hover:bg-green-500 text-black font-mono text-sm font-bold rounded border border-green-400 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/20 flex items-center">
        ...
        <div class="border border-green-600 rounded p-4 bg-gray-900">
          <!-- Result Header -->
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4 pb-4 border-b border-green-700">
            <div class="text-green-400 font-mono text-sm">
              <span class="text-green-500">{{ targetLabel }}</span>
              <div class="text-green-200 break-all">{{ results.url }}</div>
            </div>
            <div class="text-green-400 font-mono text-sm">
              <span class="text-green-500">{{ domainLabel }}</span>
              <div class="text-green-200">{{ getDomain() }}</div>
            </div>
            <div class="text-green-400 font-mono text-sm">
              <span class="text-green-500">{{ toolLabel }}</span>
              <div class="text-green-200">{{ results.tool_name }}</div>
            </div>
          </div>
          
          <!-- Command Display -->
          <div class="mb-4">
            <div class="text-green-500 font-mono text-xs mb-1">{{ commandLabel }}</div>
            <div class="text-green-300 font-mono text-sm bg-black rounded p-2 border border-green-700">
              {{ results.command }}
            </div>
          </div>

          <!-- DNS Records Section -->
          <div v-if="results.records && Object.keys(results.records).length > 0" class="space-y-3">
            <div v-for="(records, key) in results.records" :key="key" v-show="records && records.length > 0">
              <div class="text-green-500 font-mono text-xs mb-1 uppercase">{{ getRecordLabel(key) }}</div>
              <div class="space-y-1">
                <div v-for="(record, idx) in records" :key="idx" class="text-green-300 font-mono text-sm bg-black rounded px-2 py-1 border-l-2 border-green-500">
                  {{ record }}
                </div>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    // Default labels (will be overridden by backend response)
    const labels = ref({
      scanTitle: 'DNS RECONNAISSANCE',
      sessionLabel: 'SESSION',
      enterTargetLabel: 'Enter target URL:',
      placeholderText: 'https://target-system.com',
      startScanLabel: 'START DNS SCAN',
      scanLabel: 'SCAN',
      scanningLabel: 'SCANNING...',
      processingMessage: 'ANALYZING DNS RECORDS',
      executingLabel: 'EXECUTING DNS RECONNAISSANCE',
      errorTitle: 'DNS SCAN FAILED',
      resultsTitle: 'DNS SCAN RESULTS',
      downloadLabel: '[DOWNLOAD]',
      targetLabel: 'Target:',
      domainLabel: 'Domain:',
      toolLabel: 'Tool:',
      commandLabel: 'Command:',
      successMessage: 'DNS RECONNAISSANCE COMPLETED',
      errorMessage: 'DNS SCAN FAILED'
    })
    ...
    const commandDisplay = computed(() => {
      ...
      return domain ? `dnsrecon -d ${domain} -t std` : 'dnsrecon -d <target> -t std'
    })
    ...
    const getRecordLabel = (key) => {
      ...
      return labels.value[key] || labelMap[key] || key
    }
