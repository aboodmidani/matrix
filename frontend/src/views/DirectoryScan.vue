<template>
  <div class="bg-black border-2 border-blue-500 rounded-lg p-6 shadow-2xl">
    <div class="flex items-center mb-4 text-blue-400 text-sm font-mono">
      ...
      <span class="ml-2">DIRECTORY ENUMERATION</span>
    ...
    <div class="border border-blue-600 rounded p-4 md:p-6 bg-gray-900 mb-6">
      <div class="text-blue-400 font-mono text-sm mb-3">
        <span class="text-blue-300">></span> Enter target URL:
      </div>
      <div class="flex flex-col sm:flex-row gap-3">
        <input v-model="targetUrl" type="text" placeholder="https://target-system.com" class="flex-1 bg-black border border-blue-500 rounded px-3 py-3 md:py-2 text-blue-400 font-mono text-sm md:text-base focus:outline-none focus:border-blue-400 transition-colors" />
        <button @click="startScan" :disabled="loading || !targetUrl.trim()" class="px-6 py-3 md:py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-600 text-black font-mono font-bold rounded border border-blue-400 hover:border-blue-300 transition-all duration-300 disabled:cursor-not-allowed min-h-[44px] touch-manipulation">
    ...
    <div v-if="loading" class="mt-8 bg-black border-2 border-blue-500 rounded-lg p-6">
      <div class="text-blue-400 font-mono text-lg font-bold tracking-wider mb-4">
        <span class="text-blue-300">[DIRECTORY SCAN IN PROGRESS]</span>
      </div>
      <div class="border border-blue-600 rounded p-4 bg-gray-900">
        <div class="text-blue-400 font-mono text-xs mb-2">
          <span class="text-blue-300">root@matrix</span>:<span class="text-blue-400">~</span>$ dirsearch -u {{ targetUrl }}
        </div>
    ...
    <div v-if="error" class="mt-8 bg-black border-2 border-red-500 rounded-lg p-6">
      ...
      <div class="border border-red-600 rounded p-4 bg-gray-900">
        <div class="text-red-400 font-mono text-xs mb-2">
          <span class="text-red-300">root@matrix</span>:<span class="text-blue-400">~</span>$ dirsearch -u {{ targetUrl }}
        </div>
    ...
    <!-- Results -->
    <div v-if="results" class="mt-8 bg-black bg-opacity-80 border border-blue-500 rounded-lg p-6">
      ...
      <div class="border border-blue-600 rounded p-4 bg-gray-900">
        <div class="text-blue-400 font-mono text-sm mb-2">
          <span class="text-blue-300">Target:</span> {{ results.url }}
        </div>
        <div class="text-blue-400 font-mono text-sm mb-2">
          <span class="text-blue-300">Command:</span> {{ results.command }}
        </div>
        <div v-if="results.success" class="text-blue-300 font-mono text-sm">
          <div class="text-green-400 font-bold">[SUCCESS]</div>
          <pre class="mt-2 p-2 bg-black border border-blue-600 rounded text-xs overflow-auto max-h-96">{{ results.output }}</pre>
        </div>
  ...
  setup() {
    ...
    const startScan = async () => {
      ...
      try {
        ...
        error.value = err.message || 'Directory scan failed'
      } finally {
        loading.value = false
      }
    }
    ...
    const downloadReport = async () => {
      ...
      try {
        // Use apiService which respects environment variables
        await apiService.downloadReport(results.value, 'directories')
      } catch (error) {
