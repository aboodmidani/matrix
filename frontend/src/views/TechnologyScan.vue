<template>
  <div class="bg-black border-2 border-purple-500 rounded-lg p-6 shadow-2xl">
    <div class="flex items-center mb-4 text-purple-400 text-sm font-mono">
      ...
      <span class="ml-2">TECHNOLOGY DETECTION</span>
    ...
    <div class="border border-purple-600 rounded p-4 md:p-6 bg-gray-900 mb-6">
      <div class="text-purple-400 font-mono text-sm mb-3">
        <span class="text-purple-300">></span> Enter target URL:
      </div>
      <div class="flex flex-col sm:flex-row gap-3">
        <input v-model="targetUrl" type="text" placeholder="https://target-system.com" class="flex-1 bg-black border border-purple-500 rounded px-3 py-3 md:py-2 text-purple-400 font-mono text-sm md:text-base focus:outline-none focus:border-purple-400 transition-colors" />
        <button @click="startScan" :disabled="loading || !targetUrl.trim()" class="px-6 py-3 md:py-2 bg-purple-600 hover:bg-purple-500 disabled:bg-gray-600 text-black font-mono font-bold rounded border border-purple-400 hover:border-purple-300 transition-all duration-300 disabled:cursor-not-allowed min-h-[44px] touch-manipulation">
          ...
          <span v-else>
            <span class="hidden sm:inline">START TECHNOLOGY SCAN</span>
    ...
    <div v-if="loading" class="mt-8 bg-black border-2 border-purple-500 rounded-lg p-6">
      <div class="text-purple-400 font-mono text-lg font-bold tracking-wider mb-4">
        <span class="text-purple-300">[TECHNOLOGY SCAN IN PROGRESS]</span>
      </div>
      <div class="border border-purple-600 rounded p-4 bg-gray-900">
        <div class="text-purple-400 font-mono text-xs mb-2">
          <span class="text-purple-300">root@matrix</span>:<span class="text-blue-400">~</span>$ wappalyzer {{ targetUrl }}
        </div>
        <div class="w-full bg-gray-700 h-2 rounded">
          <div class="bg-purple-500 h-2 rounded animate-pulse" style="width: 100%"></div>
        </div>
        <div class="text-center text-purple-300 font-mono text-xs mt-2">
    ...
    <div v-if="error" class="mt-8 bg-black border-2 border-red-500 rounded-lg p-6">
      <div class="text-red-400 font-mono text-sm mb-4">
        <span class="text-red-300">[ERROR]</span> Technology scan failed
      </div>
      <div class="border border-red-600 rounded p-4 bg-gray-900">
        <div class="text-red-400 font-mono text-xs mb-2">
          <span class="text-red-300">root@matrix</span>:<span class="text-blue-400">~</span>$ wappalyzer {{ targetUrl }}
        </div>
    ...
    <!-- Results -->
    <div v-if="results" class="mt-8 bg-black bg-opacity-80 border border-purple-500 rounded-lg p-6">
      ...
      <div class="border border-purple-600 rounded p-4 bg-gray-900">
        <div class="text-purple-400 font-mono text-sm mb-2">
          <span class="text-purple-300">Target:</span> {{ results.url }}
        </div>
        <div class="text-purple-400 font-mono text-sm mb-2">
          <span class="text-purple-300">Command:</span> {{ results.command }}
        </div>
        <div v-if="results.success" class="text-purple-300 font-mono text-sm">
          <div class="text-green-400 font-bold">[SUCCESS]</div>
          <pre class="mt-2 p-2 bg-black border border-purple-600 rounded text-xs overflow-auto max-h-96">{{ results.output }}</pre>
        </div>
