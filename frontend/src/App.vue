<template>
  ...
  >
    ...
    <div
      v-if="showDisclaimer"
      class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 backdrop-blur-sm animate__animated animate__fadeIn"
    >
      <div
        class="bg-gray-900 border-2 border-green-500 rounded-lg p-8 max-w-2xl mx-4 shadow-2xl animate__animated animate__zoomIn"
      >
    ...
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 backdrop-blur-md animate__animated animate__fadeIn"
      @click.self="showDirectoryResults = false"
    >
      <div
        class="bg-gray-950 border-2 border-green-400 rounded-xl p-8 max-w-5xl mx-4 max-h-[85vh] overflow-hidden shadow-2xl animate__animated animate__zoomIn"
        style="box-shadow: 0 25px 50px -12px rgba(0, 255, 65, 0.25), 0 0 0 1px rgba(34, 197, 94, 0.1);"
      >
        <!-- Header with gradient border -->
        <div class="relative mb-8">
          <div class="absolute inset-0 bg-gradient-to-r from-green-500/20 via-blue-500/20 to-purple-500/20 rounded-lg blur"></div>
          <div class="relative bg-gray-900 border border-green-400/50 rounded-lg p-6">
            <div class="flex justify-between items-center">
              <div>
                <h2 class="text-2xl font-bold text-green-300 font-mono tracking-wider mb-2">
        ...
        <!-- Terminal Interface -->
        <div class="bg-black border border-green-600 rounded-lg overflow-hidden">
          <!-- Terminal Header -->
          <div class="bg-gray-800 px-4 py-3 border-b border-green-600 flex items-center justify-between">
            <div class="flex items-center space-x-2">
              <div class="w-3 h-3 bg-red-500 rounded-full"></div>
              <div class="w-3 h-3 bg-yellow-500 rounded-full"></div>
              <div class="w-3 h-3 bg-green-500 rounded-full"></div>
              <span class="text-green-400 font-mono text-sm ml-4">root@matrix:~/scan-session</span>
            </div>
          ...
          <div class="p-6 font-mono text-sm">
            <div class="text-green-400 mb-4">
              <span class="text-green-300">$</span> ./dirbuster --target {{ targetUrl }} --wordlist=common.txt --live
            </div>

            <div class="bg-gray-950 rounded p-4 border border-green-500/30">
              <div v-if="liveDirectoryResults && liveDirectoryResults.length > 0" class="space-y-3">
                <div class="flex items-center justify-between mb-4">
                  <div class="text-green-400">
                    <span class="animate-pulse mr-2">⚡</span>
                    SCANNING ACTIVE - {{ liveDirectoryResults.length }} paths tested
                  </div>
                  <div class="text-yellow-400 text-xs">
                    {{ liveDirectoryResults.filter(r => r.found).length }} FOUND / {{ liveDirectoryResults.filter(r => !r.found).length }} NOT FOUND
                  </div>
                </div>
                ...
                <div class="max-h-96 overflow-y-auto space-y-2">
                  <div
                    ...
                    :class="result.found ? 'border-green-500 bg-green-500/5' : 'border-gray-600 bg-gray-800/20'"
                  >
                    <div class="flex items-center space-x-3">
                      <span :class="result.found ? 'text-green-400' : 'text-gray-400'">
                        {{ result.found ? '✓' : '✗' }}
                      </span>
                      <span class="text-green-300 font-mono">{{ result.path }}</span>
                      <span class="text-xs text-gray-500">
                        {{ result.found ? '(ACCESSIBLE)' : '(NOT FOUND)' }}
                      </span>
                    </div>
                    <div class="flex items-center space-x-2">
                      <span class="text-xs text-gray-400">{{ result.response || 'N/A' }}</span>
                      <span
                        class="px-3 py-1 rounded-full text-xs font-bold"
                        :class="result.found ? 'bg-green-600 text-black' : 'bg-gray-600 text-green-300'"
                      >
                ...
                <div class="mt-4 pt-4 border-t border-green-500/30">
                  <div class="flex justify-between text-xs text-green-400">
                    <span>Scan Progress: {{ Math.round((liveDirectoryResults.filter(r => r.checked).length / 50) * 100) }}%</span>
                    <span>{{ liveDirectoryResults.filter(r => r.found).length }} vulnerabilities detected</span>
                  </div>
                  <div class="w-full bg-gray-700 h-2 rounded-full mt-2">
                    <div
                      ...
                      :style="{ width: Math.round((liveDirectoryResults.filter(r => r.checked).length / 50) * 100) + '%' }"
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div v-else class="text-center py-12">
                <div class="inline-flex items-center space-x-2 text-green-400 mb-4">
                  ...
                  <span class="font-mono">INITIALIZING DIRECTORY ENUMERATION...</span>
                </div>
    ...
    >
      <div class="max-w-7xl mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
          ...
          <div
            class="hidden md:flex items-center space-x-6 text-xs text-green-400 font-mono"
          >
            <div class="flex items-center space-x-2">
    ...
    <main class="relative z-10 px-4 py-8 max-w-7xl mx-auto">
      ...
      <div class="bg-black border-2 border-green-500 rounded-lg p-6 mb-8 shadow-2xl">
        ...
        <div class="border border-green-600 rounded p-4 mb-6 bg-gray-900">
          ...
          <div class="text-green-400 font-mono text-xs opacity-75">
            Web Security Audit Framework v2.0 - Advanced Penetration Testing Suite
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="border border-green-600 rounded p-4 bg-gray-900">
              ...
              <div class="flex gap-3">
                <input
                  ...
                  class="flex-1 bg-black border border-green-500 rounded px-3 py-2 text-green-400 font-mono focus:outline-none focus:border-green-400 transition-colors"
                  :class="{ 'animate__animated animate__pulse': loading }"
                />
                <button
                  @click="startAudit"
                  :disabled="loading || !targetUrl.trim()"
                  class="px-6 py-2 bg-green-600 hover:bg-green-500 disabled:bg-gray-600 text-black font-mono font-bold rounded border border-green-400 hover:border-green-300 transition-all duration-300 disabled:cursor-not-allowed"
          ...
          <div class="space-y-4">
            <div class="border border-green-600 rounded p-4 bg-gray-900">
              ...
              <div class="space-y-2">
                <label class="flex items-center text-green-300 font-mono text-xs">
                  <input v-model="scanOptions.enable_dns" type="checkbox" class="mr-2 accent-green-500" />
                  [DNS] Domain enumeration
                </label>
                <label class="flex items-center text-green-300 font-mono text-xs">
                  <input v-model="scanOptions.enable_ports" type="checkbox" class="mr-2 accent-green-500" />
                  [NET] Port reconnaissance
                </label>
                <label class="flex items-center text-green-300 font-mono text-xs">
                  <input v-model="scanOptions.enable_vulns" type="checkbox" class="mr-2 accent-green-500" />
                  [VULN] Vulnerability assessment
                </label>
                <label class="flex items-center text-green-300 font-mono text-xs">
                  <input v-model="scanOptions.enable_directories" type="checkbox" class="mr-2 accent-green-500" />
      ...
      <div v-if="loading" class="bg-black border-2 border-green-500 rounded-lg p-6 mb-8 shadow-2xl">
        <div class="flex justify-between items-center mb-6">
          ...
          <button @click="stopScan" class="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-mono text-sm font-bold rounded border-2 border-red-500 hover:border-red-400 transition-all duration-300 shadow-lg hover:shadow-red-500/25">
            [TERMINATE SCAN]
          </button>
        </div>
        <div class="border border-green-600 rounded p-4 bg-gray-900">
          ...
          <div class="space-y-1">
            ...
            <div v-if="scanOptions.enable_dns" class="flex items-center text-green-300 font-mono text-xs">
              <span class="mr-2" :class="scanProgress.dns === 'DONE' ? 'text-green-400' : 'animate-spin'">
                {{ scanProgress.dns === 'DONE' ? '✓' : '⟳' }}
              </span>
              <span>DNS enumeration & reconnaissance...</span>
              <span class="ml-auto" :class="scanProgress.dns === 'DONE' ? 'text-green-400' : 'text-yellow-400'">
                [{{ scanProgress.dns }}]
              </span>
            </div>
            <div v-if="scanOptions.enable_ports" class="flex items-center text-green-300 font-mono text-xs">
              <span class="mr-2" :class="scanProgress.ports === 'DONE' ? 'text-green-400' : scanProgress.ports === 'IN PROGRESS' ? 'animate-spin' : ''">
                {{ scanProgress.ports === 'DONE' ? '✓' : scanProgress.ports === 'IN PROGRESS' ? '⟳' : '>' }}
              </span>
              <span>Port scanning & service detection...</span>
              <span class="ml-auto" :class="scanProgress.ports === 'DONE' ? 'text-green-400' : scanProgress.ports === 'IN PROGRESS' ? 'text-yellow-400' : 'text-gray-500'">
                [{{ scanProgress.ports }}]
              </span>
            </div>
            <div v-if="scanOptions.enable_vulns" class="flex items-center text-green-300 font-mono text-xs">
              <span class="mr-2" :class="scanProgress.vulnerabilities === 'DONE' ? 'text-green-400' : scanProgress.vulnerabilities === 'IN PROGRESS' ? 'animate-spin' : ''">
                {{ scanProgress.vulnerabilities === 'DONE' ? '✓' : scanProgress.vulnerabilities === 'IN PROGRESS' ? '⟳' : '>' }}
              </span>
              <span>Vulnerability assessment...</span>
              <span class="ml-auto" :class="scanProgress.vulnerabilities === 'DONE' ? 'text-green-400' : scanProgress.vulnerabilities === 'IN PROGRESS' ? 'text-yellow-400' : 'text-gray-500'">
            ...
            <div v-if="scanOptions.enable_directories" class="flex items-center text-green-300 font-mono text-xs">
              <span class="mr-2" :class="scanProgress.directories === 'DONE' ? 'text-green-400' : scanProgress.directories === 'IN PROGRESS' ? 'animate-spin' : ''">
                {{ scanProgress.directories === 'DONE' ? '✓' : scanProgress.directories === 'IN PROGRESS' ? '⟳' : '>' }}
              </span>
              <span>Directory traversal & exposure check...</span>
              <span class="ml-auto" :class="scanProgress.directories === 'DONE' ? 'text-green-400' : scanProgress.directories === 'IN PROGRESS' ? 'text-yellow-400' : 'text-gray-500'">
                [{{ scanProgress.directories }}]
              </span>
            </div>
            <div class="flex items-center text-green-300 font-mono text-xs">
              <span class="mr-2" :class="scanProgress.technologies === 'DONE' ? 'text-green-400' : scanProgress.technologies === 'IN PROGRESS' ? 'animate-spin' : ''">
                {{ scanProgress.technologies === 'DONE' ? '✓' : scanProgress.technologies === 'IN PROGRESS' ? '⟳' : '>' }}
              </span>
              <span>Technology stack fingerprinting...</span>
              <span class="ml-auto" :class="scanProgress.technologies === 'DONE' ? 'text-green-400' : scanProgress.technologies === 'IN PROGRESS' ? 'text-yellow-400' : 'text-gray-500'">
      ...
      <div v-if="results" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-black bg-opacity-80 border border-green-500 rounded-lg p-6">
          ...
          <div class="space-y-3">
            ...
            <div class="flex justify-between">
              ...
              <span class="text-green-400 font-mono text-sm">{{ results.url }}</span>
            </div>
            <div v-if="results.hostnames && results.hostnames.length > 0 && results.hostnames[0] !== 'No hostnames found'" class="flex justify-between">
              <span class="text-green-300">Hostnames:</span>
              <div class="text-green-400 font-mono text-sm max-h-20 overflow-y-auto">
                <div v-for="hostname in results.hostnames" :key="hostname" class="mb-1">{{ hostname }}</div>
              </div>
            </div>
            <div v-if="results.ip_info && !results.ip_info.error" class="space-y-2">
              <div class="flex justify-between">
                <span class="text-green-300">Location:</span>
                <span class="text-green-400">{{ results.ip_info.city }}, {{ results.ip_info.country }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-green-300">ISP:</span>
                <span class="text-green-400 font-mono">{{ results.ip_info.isp }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-black bg-opacity-80 border border-green-500 rounded-lg p-6">
          ...
          <div v-if="results.technologies && results.technologies.length > 0" class="space-y-2">
            <div v-for="tech in results.technologies" :key="tech.name" class="flex justify-between items-center p-2 bg-gray-900 rounded">
              <div class="flex items-center">
                <img v-if="tech.icon" :src="tech.icon" :alt="tech.name" class="w-6 h-6 mr-3" @error="handleIconError" />
                <div>
                  <span class="text-green-300 text-sm">{{ tech.category }}</span>
                  <div class="text-green-400 font-mono">{{ tech.name }}</div>
                </div>
              </div>
              <span class="text-xs px-2 py-1 rounded" :class="tech.confidence === 'High' ? 'bg-green-600' : 'bg-yellow-600'">
                {{ tech.confidence }}
              </span>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <div v-if="results.ssl_info">
              <div class="flex justify-between items-center">
                ...
                <span class="px-2 py-1 rounded text-xs" :class="results.ssl_info.valid ? 'bg-green-600' : 'bg-red-600'">
            ...
            <div>
              ...
              <div class="mt-2 space-y-1">
                <div v-for="(value, header) in results.headers" :key="header" class="text-xs">
                  <span class="text-green-400">{{ header }}:</span>
                  <span class="text-green-300 ml-2" :class="value ? 'text-green-400' : 'text-red-400'">
                    {{ value ? "✓" : "✗" }}
                  </span>
                </div>
              </div>
        ...
        <div class="bg-black bg-opacity-80 border border-green-500 rounded-lg p-6">
          ...
          <div v-if="securityVulnerabilities && securityVulnerabilities.length > 0 && securityVulnerabilities[0]?.type !== 'Vulnerability Scan Disabled'" class="space-y-3">
            <div v-for="vuln in securityVulnerabilities" :key="vuln.description" class="p-3 bg-gray-900 rounded border-l-4" :class="vuln.severity === 'Critical' ? 'border-red-500' : vuln.severity === 'High' ? 'border-orange-500' : 'border-yellow-500'">
              <div class="flex justify-between items-start">
                <div>
                  <span class="text-green-400 font-bold">{{ vuln.type }}</span>
                  <p class="text-green-300 text-sm mt-1">{{ vuln.description }}</p>
                </div>
                <span class="px-2 py-1 rounded text-xs font-bold" :class="vuln.severity === 'Critical' ? 'bg-red-600' : vuln.severity === 'High' ? 'bg-orange-600' : vuln.severity === 'Medium' ? 'bg-yellow-600' : 'bg-blue-600'">
                  {{ vuln.severity }}
                </span>
              </div>
            </div>
          ...
          <div v-else-if="securityVulnerabilities && securityVulnerabilities.length > 0 && securityVulnerabilities[0]?.type === 'Vulnerability Scan Disabled'" class="text-center py-8">
            <div class="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-green-400">
              <svg class="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
              </svg>
            </div>
            <p class="text-green-400 text-lg font-bold font-mono tracking-wider">SCAN MODULE DISABLED</p>
            <p class="text-green-300 text-sm">Module was disabled in scan configuration</p>
          </div>
          ...
          <div v-else class="text-center py-8">
            ...
            <p class="text-green-400 text-lg font-bold font-mono tracking-wider">NO SECURITY VULNERABILITIES FOUND</p>
            <p class="text-green-300 text-sm">Target appears secure from common attacks</p>
          </div>
        </div>
        ...
        <div class="bg-black bg-opacity-80 border border-green-500 rounded-lg p-6">
          ...
          <div v-if="directoryResults && directoryResults.length > 0 && directoryResults[0].type !== 'Directory Scan Disabled'" class="space-y-3">
            <div v-for="result in directoryResults" :key="result.description" class="p-3 bg-gray-900 rounded border-l-4" :class="result.type === 'Directory Exposure' ? 'border-orange-500' : 'border-blue-500'">
              <div class="flex justify-between items-start">
                <div>
                  <span class="text-green-400 font-bold">{{ result.type }}</span>
                  <p class="text-green-300 text-sm mt-1">{{ result.description }}</p>
                </div>
                <span class="px-2 py-1 rounded text-xs font-bold" :class="result.severity === 'Medium' ? 'bg-orange-600' : result.severity === 'High' ? 'bg-red-600' : 'bg-blue-600'">
                  {{ result.severity }}
                </span>
              </div>
            </div>
          ...
          <div v-else-if="directoryResults && directoryResults.length > 0 && directoryResults[0].type === 'Directory Scan Disabled'" class="text-center py-8">
            <div class="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-green-400">
              <svg class="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
              </svg>
        ...
        <!-- Network Information -->
        <div class="bg-black bg-opacity-80 border border-green-500 rounded-lg p-6 lg:col-span-2">
          <h3 class="text-xl font-bold text-green-400 mb-4 flex items-center">
            <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div v-if="results.ports && results.ports.length > 0" class="space-y-2">
                <div v-for="port in results.ports" :key="port.port" class="flex justify-between items-center p-2 bg-gray-900 rounded">
                  <div>
                    <span class="text-green-400 font-mono">{{ port.port }}/{{ port.protocol || "tcp" }}</span>
                    <span class="text-green-300 ml-2">{{ port.service }}</span>
                  </div>
                  <span class="px-2 py-1 bg-green-600 text-black text-xs rounded">OPEN</span>
                </div>
              </div>
            ...
            <div>
              ...
              <div v-if="results.dns && results.dns.A_records" class="space-y-2">
                <div v-for="record in results.dns.A_records" :key="record" class="p-2 bg-gray-900 rounded">
                  <span class="text-green-400 font-mono">{{ record }}</span>
                </div>
              </div>
              <div v-else class="text-green-300">{{ results.dns?.note || "No DNS records found" }}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    async startAudit() {
      ...
      try {
        ...
        const hasProtocol = this.targetUrl.startsWith('http://') || this.targetUrl.startsWith('https://');
        const protocol = this.targetUrl.startsWith('http://') ? 'HTTP' :
                        this.targetUrl.startsWith('https://') ? 'HTTPS' : 'HTTPS (auto)';
    ...
    simulateScanProgress() {
      ...
      if (this.scanOptions.enable_vulns) this.scanProgress.vulnerabilities = 'PENDING';
      if (this.scanOptions.enable_directories) this.scanProgress.directories = 'PENDING';
...
button:focus {
  outline: none;
  box-shadow: 0 0 0 2px rgba(0, 255, 65, 0.5), 0 0 0 4px rgba(0, 255, 65, 0.2);
}
